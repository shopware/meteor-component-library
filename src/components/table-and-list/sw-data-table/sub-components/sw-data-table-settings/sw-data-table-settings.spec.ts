...
describe("sw-data-table-settings", () => {
  ...
  it("should render the base view by default", async () => {
    ...
    expect(popoverContent?.querySelector('.sw-popover__header')?.textContent).toBe('sw-data-table-settings.title');
    ...
    expect(popoverItems?.[4]?.textContent).toContain('sw-data-table-settings.frameOutlinesMetaCopy');
  ...
  it("should switch to columns view", async () => {
    ...
    const columnsPopoverItemLabel = popoverContent?.querySelectorAll('.sw-popover-item__label')?.[0];
    expect(columnsPopoverItemLabel?.textContent).toContain('sw-data-table-settings.columnOrder.title');
    ...
    expect(popoverContent?.querySelector('.sw-popover__header')?.textContent).toBe('sw-data-table-settings.columnOrder.title');
  ...
  it("should emit the correct events when the columns are changed", async () => {
    ...
    // Click on "Columns" label to switch to the columns view
    const columnsPopoverItemLabel = document.querySelector('.sw-popover__content')?.querySelectorAll('.sw-popover-item__label')?.[0];
    await columnsPopoverItemLabel?.dispatchEvent(new Event('click'));
  ...
  it("should change the visibility of all non-pinned column items to false when click on group header of visibility", async () => {
    ...
    const visibleGroupHeader = document.querySelectorAll('.sw-popover-item-result__group-action')?.[0];
    expect(visibleGroupHeader?.textContent).toContain('sw-data-table-settings.columnOrder.columnGroups.actionLabelShown');
  ...
  it("should change the visibility of all non-pinned column items to true when click on group header of hidden", async () => {
    ...
    // Trigger a group change from sw-popover-item-result
    const visibleGroupHeader = document.querySelectorAll('.sw-popover-item-result__group-action')?.[1];
    expect(visibleGroupHeader?.textContent).toContain('sw-data-table-settings.columnOrder.columnGroups.actionLabelHidden');
    await visibleGroupHeader?.dispatchEvent(new Event('click'));
